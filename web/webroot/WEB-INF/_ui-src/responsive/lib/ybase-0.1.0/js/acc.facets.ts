const facets = {
  _autoload: [['init', $('#product-facet').length]],
  facetsTemplate: '',
  baseURL: '',
  searchTerm: '',
  sort: '',
  ...
  checkSubcategory() {
    $('.js-facetTitle').each(() => {
      const $levelOneCategory = $('.js-facetCheckbox[data-facetCode="levelOneCategories"]');
      const $levelTwoCategory = $('.js-facetTitle[data-facetCode="levelTwoCategories"]');
      let flag = false;
  ...
  setNutsAllergens() {
    const $nutsAllergen = $('.js-facetListItem[data-facet-value*="Nuts"]');
    $nutsAllergen.find('form').addClass('facet__list-item--left');
    const $nutsAllergensItem = $('.js-facetTitle[data-facetCode="nutsallergens"]');
    this.NUTSALLERGENSLIST = $nutsAllergensItem
      .next('.js-facet-values')
      .find('.facet__list-item')
      .addClass('hide')
      .css('margin-left', 20);
    if ($nutsAllergen.find('.icon').length < 1) {
      $nutsAllergen.append('<span class="js-nutsAllergen icon icon--sm icon-chevron-down"></span>');
    }
    ...
    const productFacet = document.getElementById('product-facet');
    productFacet
      ?.querySelectorAll('.facet-applied [data-facet-code="nutsallergens"]')
      .forEach((appliedFacet: any): void => {
        [...this.NUTSALLERGENSLIST].forEach((nutAllergenList: any): void => {
          if (appliedFacet.dataset['facetValueCode'] === nutAllergenList.dataset['facetValue']) {
            nutAllergenList.querySelector('.js-facetCheckbox').checked = true;
          }
        });
      });
    ...
    $('.js-facetListItem').each(function (i, element) {
      const dataFacetCode = element.dataset['facetValue'];
      const facetCheckbox = element.querySelector('.js-facetCheckbox');
    ...
    const $nutsFacetCheckbox = this.NUTSALLERGENSLIST.find('.js-facetCheckbox');
    if ($isNutsChecked) {
  ...
  facetCheckHandler(e: Event) {
    const target = e.currentTarget as HTMLElement;
    const $facetCheckbox = $(target);
    ...
    this.getResults(resultsURL(), false);
  },
  ...
  removeAppliedFacet(e: Event) {
    ...
    const isOutsideOverlay = currentTarget.closest('#applied-filters') ? true : false;

    this.facetsToApply.splice(facetIndex, 1);
    this.facetsApplied = this.facetsToApply;
    this.getResults(`${resultUrl}?${decodedRemoveFacetUrl}`, isOutsideOverlay);
  },

  clearAllFacetHandler(e: Event): void {
    const target = e.target as HTMLElement;
    const url = ACC.config.promoPage ? '/my-promo-products' : (this.baseURL as string);
    const searchTerm = this.calcUrlSearchterm() as string;
    const updateUrl = `${url}${ACC.config.resultFacet}?q=${searchTerm}:${this.sort}` as string;
    const resultUrl = ACC.config.recentPage ? `${updateUrl}&weeksInPast=${this.weeksInPast}` : (updateUrl as string);
    const isOutsideOverlay = target.closest('#applied-filters') ? true : false;

    this.facetsToApply = [];
    this.getResults(resultUrl, isOutsideOverlay);
  },

  getResults(url: string, isOutsideOverlay: boolean) {
    $('.js_spinner').show();
    return $.ajax({
      type: 'GET',
      url: url,
      ...
      success(response) {
        ...
        $('.js_spinner').hide();
        const facetApplyButton = document.querySelector('.js-facetsApply') as HTMLButtonElement;
        facetApplyButton.disabled = isOutsideOverlay;
      },
      error() {
