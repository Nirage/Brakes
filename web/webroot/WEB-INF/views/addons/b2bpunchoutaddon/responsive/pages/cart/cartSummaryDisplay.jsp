           ...
           <c:if test="${!empty punchoutUser}">
            ...
            <c:if test="${empty punchoutUser}">
                    ...
                    <div class="js-orderBtnsDesktop">
                      ...
                      <c:choose>
                      <c:when test="${isPaymentEnabled && (paymentCardStatus eq 'EXPIRED' || paymentCardStatus eq 'NOT_ADDED_BUT_MANDATORY')}">
                        <button id="checkout.summary.placeOrder" name="checkout.summary.placeOrder" class="submitButtonText js-PlaceOrder btn btn-primary btn--full-width h-space-1 ${checkoutBtnClass}" disabled type="submit" tabindex="0" title="<spring:message code='checkout.summary.placeOrder'/>" ><spring:message code="checkout.summary.placeOrder"/></button>
                       </c:when>
