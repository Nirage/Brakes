                        ...
                        <div class="col-xs-12 col-sm-4 col-md-4">
                            <div class="form-group">
                                ...
                                <c:if test="${not empty orderStatuFilters}">
                                   <form id="filterOprin${top ? '1' : '2'}" name="filterOprin${top ? '1' : '2'}" method="get"
                                      action="#">
                                    <select id="filterOptions${top ? '1' : '2'}" name="statue" class="form-control">
                                    		// whenever we user select 'Waiting for confirmation' we have to update 'QUEUED' checkbox with a hidden filed in Js Level
											<c:forEach var="orderStatus" items="${orderStatuFilters}">
											<c:choose>
											<c:when test="${orderStatus.key == 'QUEUED' }">
										  <input type="checkbox" name="status" value=" ${orderStatus.key}" ${orderStatus.value? 'checked="checked"':'' } hidden="true"/>
											</c:when>
											<c:otherwise>
											  <input type="checkbox" name="status" value=" ${orderStatus.key}" ${orderStatus.value? 'checked="checked"':'' }>${orderStatus.key}<br>
											</c:otherwise>
											</c:choose>
											</c:forEach>
                                    </select>
