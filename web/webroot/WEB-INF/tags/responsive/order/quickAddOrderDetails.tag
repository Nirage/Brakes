...
<div class="quick-add ${cssClass}">
  <form:form method="post" class="quick-add__form js-quickAddForm js-orderQuickAddForm" data-large-qty="${cartLargeQuantity}" data-max-qty="${cartMaximumQuantity}"  modelAttribute="quickOrderForm" action="${contextPath}/my-account/addToAmendOrder">
    ...
    <div class="quick-add__inputs-group">
      ...
      <input type="text" id="quickOrderForm.orderCode" name="orderCode" value="${orderDetails.order.code}" hidden="true" />
    </div>
    ...
    <c:if test="${not empty isProductUnavailable}">
      ...
      <script>window.outOfStockError = true;</script>
    </c:if>
  </form:form>
</div>