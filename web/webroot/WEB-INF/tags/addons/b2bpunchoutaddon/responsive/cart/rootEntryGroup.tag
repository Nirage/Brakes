    ...
    <c:choose>
        <c:when test="${not empty entryGroup.children}">
            <c:forEach items="${entryGroup.children}" var="group" varStatus="loop">
                <addoncart:entryGroup cartData="${cartData}" entryGroup="${group}"/>
            </c:forEach>
        </c:when>
        <c:otherwise>
                    ...
                    <c:choose>
                        <c:when test="${viewPromotions && (entry.promoGroupNumber gt -1)}">
                            <div class="cart-item__promo-group">
                                <addoncart:cartItem cartData="${cartData}" entry="${entry}" index="${entryGroup.groupNumber}"/>
                                <addoncart:cartItem cartData="${cartData}" entry="${entry.linkedPromoEntry}" promo="true"/>
                            </div>
                        </c:when>
                        <c:otherwise>
                            <addoncart:cartItem cartData="${cartData}" entry="${entry}" index="${entryGroup.groupNumber}"/>
