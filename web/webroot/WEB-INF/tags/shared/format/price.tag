...
<%@ attribute name="removeSpacing" required="false" type="java.lang.Boolean" %>
...
<c:choose>
	<c:when test="${priceData.value > 0}">
		...
		<c:if test="${not removeSpacing || empty removeSpacing}">
			${formattedPrice}
		</c:if>
		<c:if test="${removeSpacing && not empty removeSpacing}">
			${fn:replace(formattedPrice, " ", "")}
		</c:if>
	</c:when>
	<c:otherwise>
		<c:if test="${displayFreeForZero}">
			<span class="js-displayingFreeForZero hide"></span><spring:theme code="text.free" text="FREE"/>
		</c:if>
