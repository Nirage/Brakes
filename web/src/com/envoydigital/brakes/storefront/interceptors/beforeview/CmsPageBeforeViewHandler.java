...
import com.envoydigital.brakes.facades.BrakesMonetateFacade;
...
import com.envoydigital.brakes.storefront.filters.cms.CMSSiteFilter;
...
public class CmsPageBeforeViewHandler implements BeforeViewHandler
{
	...
	private static final String SITE_PATH_SEPARATOR = "/";
	...
	@Resource(name = "brakesB2BUnitService")
	...
	@Resource(name = "brakesMonetateFacade")
	private BrakesMonetateFacade brakesMonetateFacade;
	...
	public void beforeView(final HttpServletRequest request, final HttpServletResponse response, final ModelAndView modelAndView)
	{
		...
		cartAvailibilty(modelAndView);
		modelAndView.addObject("cmsSite", currentSite);
		modelAndView.addObject("isb2cSite", SiteChannel.B2C.equals(currentSite.getChannel()));
		modelAndView.getModel().computeIfAbsent("isHomePage", k -> false);
		...
		modelAndView.addObject("cartLargeQuantity",
				configurationService.getConfiguration().getInt("cart.large.quantity.value", 100));
		modelAndView.addObject("cartMaximumQuantity", brakesCartFacade.getCartMaximumQuantity(currentSite));
		modelAndView.addObject("pageBodyCssClasses", buildCssClasses(page));
		if (page != null)
		{
			final boolean showPaymentBanner = brakesPaymentFacade.showPaymentBanner(page);
			modelAndView.addObject("showPaymentBanner", showPaymentBanner);
			if (showPaymentBanner)
			{
				final boolean isCardAdded = modelAndView.getModel().containsKey("isCardAdded");
				modelAndView.addObject("paymentCardStatus", brakesPaymentFacade.getPaymentCardStatus(isCardAdded));
			}
		...
		if (!brakesUserFacade.isAnonymousUser() && brakesCartFacade.hasSessionCart())
		{
			modelAndView.addObject("cartMonetateData", brakesMonetateFacade.getMonetateCartData());
		}
		...
		{
			modelAndView.addObject("currentB2BUnit", brakesB2BUnitFacade.getCurrentB2BUnit());
		}
		...
		setUpHrefLangData(request, response, modelAndView, currentSite);


		final String releaseVersion = configurationService.getConfiguration().getString("build.version");
		if (StringUtils.isNotEmpty(releaseVersion))
		{
			final String[] parts = releaseVersion.split("-");
			if (parts.length > 3)
			{
				modelAndView.addObject("releaseVersion", parts[3]);
			}
		}
	}

	protected void setUpHrefLangData(final HttpServletRequest request, final HttpServletResponse response,
			final ModelAndView modelAndView, final CMSSiteModel currentSite)
	{
		String canonicalFlag = null;
		String hrefUrl = null;

		final String hybrisUrl = request.getRequestURI().substring(request.getContextPath().length());
		final String url = request.getRequestURL().toString();
		final SEOData seoData = seoFacade.getSEOURLForHybrisURL(hybrisUrl, currentSite);

		if (seoData != null)
		{
			final String replacablePath = hybrisUrl;
			hrefUrl = url.replace(replacablePath, seoData.getTargetUrl());
			canonicalFlag = hrefUrl;
			modelAndView.addObject("canonicalFlag", canonicalFlag);
		}
	}
	...
	public void cartAvailibilty(final ModelAndView model)
	{

		boolean isCartAvailableForCustomer = false;
		if (!userFacade.isAnonymousUser() && brakesCartFacade.hasSessionCart())
		{
			final CartData cart = brakesCartFacade.getMiniCart();
			isCartAvailableForCustomer = true;
			model.addObject("cartTotalItems", cart.getTotalItems());

		}
		model.addObject("isCartAvailableForCustomer", isCartAvailableForCustomer);
	}
}
