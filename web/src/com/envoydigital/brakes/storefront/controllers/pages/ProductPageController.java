...
import com.envoydigital.brakes.brakesaddon.breadcrumb.impl.BrakesProductBreadcrumbBuilder;
...
import com.envoydigital.brakes.storefront.forms.validation.BrakesEmailProductFormValidator;
import com.envoydigital.brakes.storefront.util.BrakesPageTitleResolver;
...
public class ProductPageController extends AbstractPageController
{
	...
	private BrakesProductBreadcrumbBuilder productBreadcrumbBuilder;
	...
	@Resource(name = "brakesEmailProductFormValidator")
	private BrakesEmailProductFormValidator brakesEmailProductFormValidator;

	@Resource(name = "defaultBrakesProductFacade")
	private BrakesProductFacade brakesProductFacade;

	@Resource(name = "brakesPageTitleResolver")
	private BrakesPageTitleResolver brakesPageTitleResolver;
	...
	final String encodedProductCode, final Model model, @RequestParam(value = "term", required = false)
	final String term, final HttpServletRequest request, final HttpServletResponse response)
			throws CMSItemNotFoundException, UnsupportedEncodingException
	{
		model.addAttribute("searchTerm", XSSFilterUtil.filter(term));
		final String productCode = decodeWithScheme(encodedProductCode, UTF_8);
		return prepareProductDetailPage(productCode, model, request, response);
	}

	private String prepareProductDetailPage(final String productCode, final Model model, final HttpServletRequest request,
			final HttpServletResponse response)
			throws UnsupportedEncodingException, CMSItemNotFoundException, UnknownIdentifierException
	{


		final ProductModel product = productService.getProductByCode(productCode);
		...
		populateProductDetailForDisplay(productCode, model, request, null);
		model.addAttribute(new ReviewForm());
		model.addAttribute(new EmailProductForm());
	...
	@RequestMapping(value = "/send-productdetails", method = RequestMethod.POST)
	public String sendProductDetails(@Valid
	final EmailProductForm form, final BindingResult bindingResult, final Model model, final RedirectAttributes redirectModel,
			final HttpServletRequest request, final HttpServletResponse response)
			throws CMSItemNotFoundException, UnsupportedEncodingException
	{
		form.setEmailAddress(StringUtils.trim(form.getEmailAddress()));
		brakesEmailProductFormValidator.validate(form, bindingResult);
		if (bindingResult.hasErrors())
		{
			GlobalMessages.addErrorMessage(model, "form.global.error");
			return prepareProductDetailPage(form.getEmailProductId(), model, request, response);
		}
		brakesProductFacade.emailProductDetails(form.getEmailProductId(), form.getEmailAddress());
		GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.CONF_MESSAGES_HOLDER, "product.details.emailed.successfully");

		return REDIRECT_PREFIX
				+ (request.getHeader("referer") == null ? "/p/" + form.getEmailProductId() : request.getHeader("referer"));
	...
	{
		storeContentPageTitleInModel(model, getBrakesPageTitleResolver().resolveProductPageTitle(productCode));
	}
	...
	{
		final ProductModel productModel = productService.getProductForCode(productCode, false);
		...
		final List<ProductOption> options = new ArrayList<>(
				Arrays.asList(ProductOption.BASIC, ProductOption.URL, ProductOption.PRICE, ProductOption.SUMMARY,
						ProductOption.DESCRIPTION, ProductOption.GALLERY, ProductOption.CATEGORIES, ProductOption.PROMOTIONS,
						ProductOption.CLASSIFICATION, ProductOption.STOCK, ProductOption.PRICE_RANGE,
						ProductOption.DELIVERY_MODE_AVAILABILITY, ProductOption.CART, ProductOption.ADDITIONAL_PRICE_ATTR,
						ProductOption.BRAKESPRODUCTDETAILS, ProductOption.PROMOTIONS, ProductOption.PDPICONS));


		final ProductData productData = brakesProductFacade.getProductForCodeAndOptions(productCode, options);

		sortVariantOptionData(productData);
		storeCmsPageInModel(model, getPageForProduct(productCode));
		populateProductData(productData, model);
		populateProductAdditionalData(productData, productModel);
		final String metaKeywords = MetaSanitizerUtil.sanitizeKeywords(productData.getKeywords());
		final String metaDescription = MetaSanitizerUtil.sanitizeDescription(metaDataFacade.getProductPageMetaDescription(productModel));
		setUpMetaData(model, metaKeywords, metaDescription);
	...
	protected void populateProductAdditionalData(final ProductData productData, final ProductModel productModel)
	{
		productData.setGuidanceSheet(productModel.getProductGuidanceSheet());
		productData.setSafetyDataSheet(productModel.getSafetyDataSheet());
	...
	protected List<Map<String, ImageData>> getGalleryImages(final ProductData productData)
	{
		...
		if (CollectionUtils.isNotEmpty(productData.getImages()))
		{
			...
			sortImagesBasedOnUrlPattern(images);
	...
	protected List<ImageData> sortImagesBasedOnUrlPattern(List<ImageData> imageList) {
		if(CollectionUtils.isNotEmpty(imageList)) {
			Collections.sort(imageList, new Comparator<ImageData>() {
				@Override
				public int compare(ImageData im1, ImageData im2) {
					if (im1.getCleanUrl().split("_").length > 1
							&& im2.getCleanUrl().split("_").length > 1) {
						try {
							int image1Index = Integer.parseInt(im1.getCleanUrl().split("_")[1]);
							int image2Index = Integer.parseInt(im2.getCleanUrl().split("_")[1]);

							return (image1Index > image2Index) ? 1 : -1;
	...
	/**
	 * @return the brakesPageTitleResolver
	 */
	public BrakesPageTitleResolver getBrakesPageTitleResolver()
	{
		return brakesPageTitleResolver;
	}

	/**
	 * @param brakesPageTitleResolver
	 *           the brakesPageTitleResolver to set
	 */
	public void setBrakesPageTitleResolver(final BrakesPageTitleResolver brakesPageTitleResolver)
	{
		this.brakesPageTitleResolver = brakesPageTitleResolver;
	}
