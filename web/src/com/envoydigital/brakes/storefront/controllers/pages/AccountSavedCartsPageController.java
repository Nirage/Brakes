...
public class AccountSavedCartsPageController extends AbstractSearchPageController
{
	...
	@RequireHardLogIn
	public String savedCarts(@RequestParam(value = "page", defaultValue = "1")
	...
	@RequireHardLogIn
	public RedirectView restoreSaveCartForId(@RequestHeader(value = "referer", required = false)
	final String referer, @PathVariable(value = "cartId")
	final String cartId, final RedirectAttributes redirectModel, final Model model)
			throws CommerceSaveCartException, CommerceCartModificationException
	{
		try
		{
			// reloading saved cart logic
			final CommerceSaveCartParameterData commerceSaveCartParameterData = new CommerceSaveCartParameterData();
			...
			//removing unavailable products from basket logic
			final List<CartModificationData> unavailableProducts = saveCartFacade
					.removeUnavailableProductsFromBasketAndReturnStatus(cartService.getSessionCart());
			redirectModel.addFlashAttribute("unavailableProductsFlag", CollectionUtils.isNotEmpty(unavailableProducts));

			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.CONF_MESSAGES_HOLDER, "text.restore.savedcart.sucess",
					new Object[]
					{ cartId });
		}
		catch (final Exception csce)
		{

			LOG.error(csce.getMessage(), csce);
			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER, "text.account.saveCart.edit.error",
					new Object[]
					{ cartId });
		}
