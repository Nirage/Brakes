...
public class ConfigureController extends AbstractPageController
{
			...
			throws CMSItemNotFoundException, UnsupportedEncodingException
	{
		...
		if (productData.getConfigurable())
		{
			...
			// BRAKESP2-2896 - It seems other Projects might be using the same code: if we will use this feature we
			// 				   might need to double check why the below code was outside the
			// 				   if (productData.getConfigurable()).
			//                 Actually, as for the above description, "If product is not configurable the controller
			//                 redirect to 404 page.", so we have just changed the code based on that concept.
